...
Output:
    - results/5m_tv_walkthrough/walkthrough.html (single file with all data)
...
class TradingViewFiveMinuteAnalyzer:
    """Generates TradingView-style charts for 5M timeframe walkthrough."""

    def __init__(self, output_dir=None):
        if output_dir is None:
            current_file = Path(__file__)
            project_root = current_file.parent.parent
            self.output_dir = project_root / 'results' / '5m_tv_walkthrough'
        else:
...
<head>
    <meta charset="UTF-8">
    <title>5M TradingView Walkthrough</title>
...
"""
    ...
    def run(self, df_5m):
        ...
        print(f"\n{'='*80}")
        print(f"5M TRADINGVIEW CANDLE-BY-CANDLE WALKTHROUGH")
        print(f"{'='*80}\n")
...
def main():
    ...
    # Run analysis
    analyzer = TradingViewFiveMinuteAnalyzer()
    analyzer.run(df_5m)
