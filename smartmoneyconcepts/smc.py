...
class smc:
    ...
    def fvg(cls, ohlc: DataFrame, join_consecutive=False) -> Series:
        ...
        Respected = True if FVG was mitigated but not disrespected,
                   False if disrespected (candle closed through the FVG),
                   None if not yet touched
        ...
        # Calculate Respected and StatusIndex
        n = len(ohlc)
        ...
        for i in np.where(~np.isnan(fvg))[0]:
            # Check if FVG was disrespected (candle closed through it)
            if fvg[i] == 1:  # Bullish FVG
                # Disrespected if any future candle closes below the bottom
                future_closes = close_prices[i + 2:]
                disrespected_mask = future_closes < bottom[i]
                if np.any(disrespected_mask):
                    respected[i] = False
                    status_index[i] = i + 2 + np.argmax(disrespected_mask)
                # Respected if mitigated but not disrespected
                elif mitigated_index[i] != 0:
                    respected[i] = True
                    status_index[i] = int(mitigated_index[i])
                # Still pending
                else:
                    respected[i] = None
                    status_index[i] = 0
                ...
                # Disrespected if any future candle closes above the top
                future_closes = close_prices[i + 2:]
                disrespected_mask = future_closes > top[i]
                if np.any(disrespected_mask):
                    respected[i] = False
                    status_index[i] = i + 2 + np.argmax(disrespected_mask)
                # Respected if mitigated but not disrespected
        ...
        return pd.concat(
            [
                ...
                pd.Series(respected, name="Respected"),
                pd.Series(status_index, name="StatusIndex"),
            ],
            axis=1,
        )
